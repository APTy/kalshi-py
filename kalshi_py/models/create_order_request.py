from collections.abc import Mapping
from typing import Any, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..models.create_order_request_action import CreateOrderRequestAction
from ..models.create_order_request_self_trade_prevention_type import CreateOrderRequestSelfTradePreventionType
from ..models.create_order_request_side import CreateOrderRequestSide
from ..models.create_order_request_time_in_force import CreateOrderRequestTimeInForce
from ..models.create_order_request_type import CreateOrderRequestType
from ..types import UNSET, Unset

T = TypeVar("T", bound="CreateOrderRequest")


@_attrs_define
class CreateOrderRequest:
    """
    Attributes:
        ticker (str):
        side (CreateOrderRequestSide):
        action (CreateOrderRequestAction):
        count (int):
        client_order_id (Union[Unset, str]):
        type_ (Union[Unset, CreateOrderRequestType]):  Default: CreateOrderRequestType.LIMIT.
        yes_price (Union[Unset, float]):
        no_price (Union[Unset, float]):
        yes_price_dollars (Union[Unset, str]): Submitting price of the Yes side in fixed-point dollars Example: 0.5000.
        no_price_dollars (Union[Unset, str]): Submitting price of the No side in fixed-point dollars Example: 0.5000.
        expiration_ts (Union[Unset, int]):
        time_in_force (Union[Unset, CreateOrderRequestTimeInForce]):
        buy_max_cost (Union[Unset, int]): Maximum cost in cents. When specified, the order will automatically have Fill-
            or-Kill (FoK) behavior.
        post_only (Union[Unset, bool]):
        reduce_only (Union[Unset, bool]):
        sell_position_floor (Union[Unset, int]): Deprecated: Use reduce_only instead. Only accepts value of 0.
        self_trade_prevention_type (Union[Unset, CreateOrderRequestSelfTradePreventionType]): The self-trade prevention
            type for this order
        order_group_id (Union[Unset, str]): The order group this order is part of
        cancel_order_on_pause (Union[Unset, bool]): If this flag is set to true, the order will be canceled if the order
            is open and trading on the exchange is paused for any reason.
    """

    ticker: str
    side: CreateOrderRequestSide
    action: CreateOrderRequestAction
    count: int
    client_order_id: Union[Unset, str] = UNSET
    type_: Union[Unset, CreateOrderRequestType] = CreateOrderRequestType.LIMIT
    yes_price: Union[Unset, float] = UNSET
    no_price: Union[Unset, float] = UNSET
    yes_price_dollars: Union[Unset, str] = UNSET
    no_price_dollars: Union[Unset, str] = UNSET
    expiration_ts: Union[Unset, int] = UNSET
    time_in_force: Union[Unset, CreateOrderRequestTimeInForce] = UNSET
    buy_max_cost: Union[Unset, int] = UNSET
    post_only: Union[Unset, bool] = UNSET
    reduce_only: Union[Unset, bool] = UNSET
    sell_position_floor: Union[Unset, int] = UNSET
    self_trade_prevention_type: Union[Unset, CreateOrderRequestSelfTradePreventionType] = UNSET
    order_group_id: Union[Unset, str] = UNSET
    cancel_order_on_pause: Union[Unset, bool] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        ticker = self.ticker

        side = self.side.value

        action = self.action.value

        count = self.count

        client_order_id = self.client_order_id

        type_: Union[Unset, str] = UNSET
        if not isinstance(self.type_, Unset):
            type_ = self.type_.value

        yes_price = self.yes_price

        no_price = self.no_price

        yes_price_dollars = self.yes_price_dollars

        no_price_dollars = self.no_price_dollars

        expiration_ts = self.expiration_ts

        time_in_force: Union[Unset, str] = UNSET
        if not isinstance(self.time_in_force, Unset):
            time_in_force = self.time_in_force.value

        buy_max_cost = self.buy_max_cost

        post_only = self.post_only

        reduce_only = self.reduce_only

        sell_position_floor = self.sell_position_floor

        self_trade_prevention_type: Union[Unset, str] = UNSET
        if not isinstance(self.self_trade_prevention_type, Unset):
            self_trade_prevention_type = self.self_trade_prevention_type.value

        order_group_id = self.order_group_id

        cancel_order_on_pause = self.cancel_order_on_pause

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "ticker": ticker,
                "side": side,
                "action": action,
                "count": count,
            }
        )
        if client_order_id is not UNSET:
            field_dict["client_order_id"] = client_order_id
        if type_ is not UNSET:
            field_dict["type"] = type_
        if yes_price is not UNSET:
            field_dict["yes_price"] = yes_price
        if no_price is not UNSET:
            field_dict["no_price"] = no_price
        if yes_price_dollars is not UNSET:
            field_dict["yes_price_dollars"] = yes_price_dollars
        if no_price_dollars is not UNSET:
            field_dict["no_price_dollars"] = no_price_dollars
        if expiration_ts is not UNSET:
            field_dict["expiration_ts"] = expiration_ts
        if time_in_force is not UNSET:
            field_dict["time_in_force"] = time_in_force
        if buy_max_cost is not UNSET:
            field_dict["buy_max_cost"] = buy_max_cost
        if post_only is not UNSET:
            field_dict["post_only"] = post_only
        if reduce_only is not UNSET:
            field_dict["reduce_only"] = reduce_only
        if sell_position_floor is not UNSET:
            field_dict["sell_position_floor"] = sell_position_floor
        if self_trade_prevention_type is not UNSET:
            field_dict["self_trade_prevention_type"] = self_trade_prevention_type
        if order_group_id is not UNSET:
            field_dict["order_group_id"] = order_group_id
        if cancel_order_on_pause is not UNSET:
            field_dict["cancel_order_on_pause"] = cancel_order_on_pause

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        ticker = d.pop("ticker")

        side = CreateOrderRequestSide(d.pop("side"))

        action = CreateOrderRequestAction(d.pop("action"))

        count = d.pop("count")

        client_order_id = d.pop("client_order_id", UNSET)

        _type_ = d.pop("type", UNSET)
        type_: Union[Unset, CreateOrderRequestType]
        if isinstance(_type_, Unset):
            type_ = UNSET
        else:
            type_ = CreateOrderRequestType(_type_)

        yes_price = d.pop("yes_price", UNSET)

        no_price = d.pop("no_price", UNSET)

        yes_price_dollars = d.pop("yes_price_dollars", UNSET)

        no_price_dollars = d.pop("no_price_dollars", UNSET)

        expiration_ts = d.pop("expiration_ts", UNSET)

        _time_in_force = d.pop("time_in_force", UNSET)
        time_in_force: Union[Unset, CreateOrderRequestTimeInForce]
        if isinstance(_time_in_force, Unset):
            time_in_force = UNSET
        else:
            time_in_force = CreateOrderRequestTimeInForce(_time_in_force)

        buy_max_cost = d.pop("buy_max_cost", UNSET)

        post_only = d.pop("post_only", UNSET)

        reduce_only = d.pop("reduce_only", UNSET)

        sell_position_floor = d.pop("sell_position_floor", UNSET)

        _self_trade_prevention_type = d.pop("self_trade_prevention_type", UNSET)
        self_trade_prevention_type: Union[Unset, CreateOrderRequestSelfTradePreventionType]
        if isinstance(_self_trade_prevention_type, Unset):
            self_trade_prevention_type = UNSET
        else:
            self_trade_prevention_type = CreateOrderRequestSelfTradePreventionType(_self_trade_prevention_type)

        order_group_id = d.pop("order_group_id", UNSET)

        cancel_order_on_pause = d.pop("cancel_order_on_pause", UNSET)

        create_order_request = cls(
            ticker=ticker,
            side=side,
            action=action,
            count=count,
            client_order_id=client_order_id,
            type_=type_,
            yes_price=yes_price,
            no_price=no_price,
            yes_price_dollars=yes_price_dollars,
            no_price_dollars=no_price_dollars,
            expiration_ts=expiration_ts,
            time_in_force=time_in_force,
            buy_max_cost=buy_max_cost,
            post_only=post_only,
            reduce_only=reduce_only,
            sell_position_floor=sell_position_floor,
            self_trade_prevention_type=self_trade_prevention_type,
            order_group_id=order_group_id,
            cancel_order_on_pause=cancel_order_on_pause,
        )

        create_order_request.additional_properties = d
        return create_order_request

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
